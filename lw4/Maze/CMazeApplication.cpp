...
void CMazeApplication::OnKeyboard(unsigned char key, int x, int y)
{
	...
	case 'w':
		...
		if (m_observer.GetDirectionZ() <= 0 && m_observer.GetDirectionX() >= 0)
		{
			...
			leftBorderX = floor((m_observer.GetCoordX() + m_observer.GetDirectionX() * m_observer.GetSpeed() + Z_NEAR * 2) / 2);
			...
			frontBorderX = floor((m_observer.GetCoordX() + m_observer.GetDirectionX() * m_observer.GetSpeed() + Z_NEAR * 2) / 2);
		}
		// если идём в +Z (или 0) и +Х (или 0), проверяем пространство вокруг на наличие преград
		...
		if (m_maze.IsCube(leftBorderZ, leftBorderX) || m_maze.IsCube(rightBorderZ, rightBorderX) || m_maze.IsCube(frontBorderZ, frontBorderX))
		{
			break;
		}
	...
	case 's':
		...
		break;
	case 'a':
		m_observer.SetAngle(m_observer.GetAngle() - 0.05f);
